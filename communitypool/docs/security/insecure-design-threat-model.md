...
| User Supabase row (auth + profile) | Drives KYC gate, pool-activity cache, watch-only address list. |
...
| Actor | Trust |
|---|---|
| Anonymous web visitor | None. Limited to public reads (fees, marketing). |
...
| Service-role admin client (server) | Trusted; used only by `/api/*` handlers. Compromise = full DB control. |
...
1. **Browser ↔ Supabase (anon key)** — RLS enforces per-user scoping. Users cannot write `pool_owner_memberships`, `user_pool_deployments`, or pool ownership rows (service-role-only after migration `20260427130000`).
...
3. **Server ↔ chain** — Reads via RPC providers, writes via the user's wallet (server never holds user keys).
4. **Wallet ↔ chain** — `assertChainMatchesExpected` (`lib/onchain/community-pool.ts`) blocks tx broadcast if wallet chain ID ≠ `NEXT_PUBLIC_EXPECTED_CHAIN_ID`.
...
### 5.3 Monetization
- No subscription, no Free/Pro tiers, no per-plan pool limit. Every authenticated user may deploy unlimited pools; the deploy preflight (`/api/pools/check-deploy`) only confirms the session.
- The product is transitioning to an on-chain protocol funding fee (Phase 2, not yet deployed). Until then 100% of every ETH / ERC-20 contribution stays in the pool and no fee is emulated client-side.
...
### 5.10 Billing lifecycle
- None. There is no checkout, billing portal, or subscription state to reconcile.
...
I-05. No application path may gate pool deployment on a plan, subscription, or pool count. Deployment requires only an authenticated session plus the existing wallet / chain / KYC-completion checks.
...
| M-06 | Deploy preflight unavailable (Supabase outage) | Modal shows a retry message and never opens the wallet prompt; no plan state exists to go stale. |
...
| T-06 | Protocol fee is not yet enforced on-chain | bryceson | Phase 1 removed subscription billing before the fee contracts ship; funding stays 100% in-pool until Phase 2. | Fees page states the model is transitional and no fee is charged yet. | Phase 2: on-chain fee + pre-transaction fee preview. |
| T-07 | Supabase service-role admin key holds full DB write access | bryceson | Required for server routes that bypass RLS. Rotation policy in `mainnet-deployment-key-policy.md`. | None (internal key). | Per-route signing, kms-backed key. |
